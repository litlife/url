...
class UrlTest extends TestCase
{
	...
	{
		$this->assertEquals('jpg', Url::fromString('Images/cover.jpg')
			->getExtension());
	...
	public function testGetQuery()
	{
		...
		$s = Url::fromString('/test/test?test=test&page=1&#item')
			->getQuery();
		$this->assertEquals('test=test&page=1', $s);
	}

	public function testGetSegments()
	{
		$ar = Url::fromString('text/Images/cover123.jpg?test=123')
			->getSegments();
		$this->assertEquals(['text', 'Images', 'cover123.jpg'], $ar);
	}
	...
	public function testGetPath()
	{
		...
		$url = Url::fromString('https://domain.com//#')
			->getPath();

		$this->assertEquals('/', $url);

		$url = Url::fromString('https://domain.com//?key=value#')
			->getPath();
	...
	{
		$url = Url::fromString('https://sebastian:super_secret@spatie.be');
		$this->assertEquals('sebastian:super_secret', $url->getUserInfo());
	}

	public function testOther()
	{
		$url = Url::fromString('https://u5a7cbca264s#');

		$this->assertEquals('https://u5a7cbca264s', (string)$url);
	}
	...
	{
		$url = Url::fromString('https://domain.com/dirname/file.txt?key=value#fragment')
			->getPathQueryFragment();

		$this->assertEquals('/dirname/file.txt?key=value#fragment', (string)$url);

		$url = Url::fromString('https://domain.com/dirname/file.txt?')
			->getPathQueryFragment();

		$this->assertEquals('/dirname/file.txt', (string)$url);

		$url = Url::fromString('https://domain.com/dirname/file.txt?key=value#')
			->getPathQueryFragment();

		$this->assertEquals('/dirname/file.txt?key=value', (string)$url);

		$url = Url::fromString('https://domain.com/dirname/?key=value#')
			->getPathQueryFragment();
		...
		$url = Url::fromString('https://domain.com/test/?key=value#fragment')
			->getPathQueryFragment();
		...
		$url = Url::fromString('https://domain.com/?key=value#fragment')
			->getPathQueryFragment();
	...
	public function testWithQueryParameter()
	{
		...
		$value = 'https://example.com/test/?key=value&foo=bar#fragment';

		$url = Url::fromString('/test')
			->withQueryParameter('key', $value);

		$this->assertEquals('/test?key=https%3A%2F%2Fexample.com%2Ftest%2F%3Fkey%3Dvalue%26foo%3Dbar%23fragment', (string)$url);
	...
	{
		$url = 'https://dev.litlife.club/папка/файл.txt?ключ=значение';

		$url = Url::fromString($url)->urlencode();

		$this->assertEquals('https://dev.litlife.club/%D0%BF%D0%B0%D0%BF%D0%BA%D0%B0/%D1%84%D0%B0%D0%B9%D0%BB.txt?%D0%BA%D0%BB%D1%8E%D1%87=%D0%B7%D0%BD%D0%B0%D1%87%D0%B5%D0%BD%D0%B8%D0%B5',
			(string)$url);
